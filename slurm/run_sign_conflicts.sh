...
for method_name in "${methods[@]}"; do
    for order_seed in "${order_seeds[@]}"; do
        for style_seed in "${style_seeds[@]}"; do
            log_file="slurm_out/conflicts_style_${method_name}_order${order_seed}_seed${style_seed}.log"
...
python ./eval/sign_conflicts_A_B_vectors.py --models_path models/${method_name}/seed_${style_seed}_style/seed_${order_seed}_order --method_name ${method_name} --task_type style
EOT
